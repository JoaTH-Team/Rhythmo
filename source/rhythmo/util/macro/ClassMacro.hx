package rhythmo.util.macro;

import haxe.macro.Context;
import haxe.macro.Expr;
import haxe.macro.Type;
import rhythmo.util.macro.MacroUtil;
...
class ClassMacro
{
	...
	public static macro function listClassesInPackage(targetPackage:String, includeSubPackages:Bool = true):ExprOf<Iterable<Class<Dynamic>>>
	{
		...
		return macro rhythmo.util.macro.CompiledClassList.get($v{request});
	}
	...
	public static macro function listSubclassesOf<T>(targetClassExpr:ExprOf<Class<T>>):ExprOf<List<Class<T>>>
	{
		...
		return macro rhythmo.util.macro.CompiledClassList.getTyped($v{request}, ${targetClassExpr});
	}
	...
	{
		final compiledClassList:ClassType = MacroUtil.getClassType('rhythmo.util.macro.CompiledClassList');
